...
class PDFtoExcelApp(ctk.CTk):
    ...
    def browse_input(self):
        filename = fd.askopenfilename(title="Select a PDF file", filetypes=[("PDF files", "*.pdf"), ("All files", "*.*")])
        if filename:
    ...
    def browse_output(self):
        filename = fd.asksaveasfilename(title="Save Excel file as", defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx"), ("All files", "*.*")])
        if filename:
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                for i, page in enumerate(pdf.pages):
                    ...
                    if strategy in ["Automatic", "Lines (Grid)"]:
                        table = page.extract_table(table_settings={"vertical_strategy": "lines", "horizontal_strategy": "lines", "snap_tolerance": 3})
                    ...
                    if table:
                        ...
                        for row_idx, row_data in enumerate(table, 1):
                            for col_idx, cell_value in enumerate(row_data, 1):
                                fixed_val = fix_arabic_text(cell_value)
                                ws.cell(row=row_idx, column=col_idx, value=fixed_val)
            if tables_found:
